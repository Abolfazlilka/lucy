...
void Lexer::identifier(){size_t s=current_-1;while(std::isalnum((unsigned char)peek())||peek()=='_')advance();auto x=source_.substr(s,current_-s);auto it=K.find(x);add(it==K.end()?TokenType::Identifier:it->second,x);}
